...
def run_fetch(url: str) -> dict:
    proc = subprocess.run(["python", "/app/app/douyin_fetch.py", url], text=True, capture_output=True, check=True)
...
def handle_message(text: str, token: str, target: str, reply_to_message_id: str | None = None):
    ...
    if data.get("content_type") == "image":
        ...
        tg.send_document(data["downloaded_media"], caption=caption, reply_to_message_id=reply_to_message_id)
    return data
